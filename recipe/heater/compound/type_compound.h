...
template < e_type T, e_type A, e_type B, class SZ > struct type_one_two_blah : type_all_flagged_of < T, SZ, BS_FN | UQ_BLAH, A, B >
{   using type_all_flagged_of < T, SZ, BS_FN | UQ_BLAH, A, B > :: type_all_flagged_of; };
...
template < e_type T, e_type P, e_type Q, e_type R, e_type S, class SZ > struct one_of_four_or_string : tidy_string < T >
{   using tidy_string < T > :: tidy_string;
    void set_value (nitpick& nits, const html_version& v, const ::std::string& s)
    {   tidy_string < T > :: set_value (nits, v, s);
        if (tidy_string < T > :: good () || tidy_string < T > :: empty ())
        {   const ::std::string ss (tidy_string < T > :: get_string ());
            nitpick nets, nuts, knots, gnats, gits;
            if (compare_complain (nets, v, SZ :: sz (), ss)) return;
            if (test_value < P > (nuts, v, ss, tidy_string < T > :: id ()))
            {   nits.merge (nuts); return; }
            if (test_value < Q > (knots, v, ss, tidy_string < T > :: id ()))
            ...
            if (test_value < R > (gnats, v, ss, tidy_string < T > :: id ()))
            {   nits.merge (gnats); return; }
            if (test_value < S > (gits, v, ss, tidy_string < T > :: id ()))
            {   nits.merge (gits); return; }
            nits.merge (nets); nits.merge (nuts); nits.merge (knots); nits.merge (gnats); nits.merge (gits); }
        tidy_string < T > :: status (s_invalid); } };
...
template < e_type T, e_type P, e_type Q, e_type R, class SZ1, class SZ2 > struct any_type_or_either_string : tidy_string < T >
{   using tidy_string < T > :: tidy_string;
    static e_animation_type animation_type () noexcept { return grab_animation_type < P > (); }
    void set_value (nitpick& nits, const html_version& v, const ::std::string& s)
    {   tidy_string < T > :: set_value (nits, v, s);
        if (tidy_string < T > :: good () || tidy_string < T > :: empty ())
        {   const ::std::string ss (tidy_string < T > :: get_string ());
            nitpick nets, gnats, nuts, knots, knights;
            if (compare_complain (nets, v, SZ1 :: sz (), ss)) { nits.merge (nets); return; }
            if (compare_complain (gnats, v, SZ2 :: sz (), ss))  { nits.merge (gnats); return; }
            if (test_value < P > (nuts, v, ss, tidy_string < T > :: id ()))
            {   nits.merge (nuts); return; }
            if (test_value < Q > (knots, v, ss, tidy_string < T > :: id ()))
            {   nits.merge (knots); return; }
            if (test_value < R > (knights, v, ss, tidy_string < T > :: id ()))
            {   nits.merge (knights); return; }
            nits.merge (nets); nits.merge (gnats); nits.merge (nuts); nits.merge (knots); nits.merge (knights); }
        tidy_string < T > :: status (s_invalid); } };
...
template < e_type T, e_type P, e_type Q, e_type R, class SZ1, class SZ2, class SZ3 > struct any_type_or_any_string : tidy_string < T >
{   using tidy_string < T > :: tidy_string;
    static e_animation_type animation_type () noexcept { return grab_animation_type < P > (); }
    void set_value (nitpick& nits, const html_version& v, const ::std::string& s)
    {   tidy_string < T > :: set_value (nits, v, s);
        if (tidy_string < T > :: good () || tidy_string < T > :: empty ())
        {   const ::std::string ss (tidy_string < T > :: get_string ());
            nitpick nets, gnats, knits, nuts, knots, aargh;
            if (compare_complain (nets, v, SZ1 :: sz (), ss)) { nits.merge (nets); return; }
            if (compare_complain (gnats, v, SZ2 :: sz (), ss)) { nits.merge (gnats); return; }
            if (compare_complain (knits, v, SZ3 :: sz (), ss)) { nits.merge (knits); return; }
            if (test_value < P > (nuts, v, ss, tidy_string < T > :: id ()))
            ...
            if (test_value < Q > (knots, v, ss, tidy_string < T > :: id ()))
            {   nits.merge (knots); return; }
            if (test_value < Q > (aargh, v, ss, tidy_string < T > :: id ()))
            {   nits.merge (aargh); return; }
            nits.merge (nets); nits.merge (gnats); nits.merge (knits); nits.merge (nuts); nits.merge (knots); nits.merge (aargh); }
        tidy_string < T > :: status (s_invalid); } };
...
template < e_type T, e_type P, class SZ1, class SZ2, class SZ3, class SZ4 > struct type_or_any_string_4 : tidy_string < T >
{   using tidy_string < T > :: tidy_string;
    static e_animation_type animation_type () noexcept { return grab_animation_type < P > (); }
    void set_value (nitpick& nits, const html_version& v, const ::std::string& s)
    {   tidy_string < T > :: set_value (nits, v, s);
...
{   bool both_ = false;
    ...
    {   vstr_t vs (uq4 < T, SZ, F > :: set_value (nits, v, s));
        ...
        {   nitpick nuts, nets;
            ...
            if ((! both_) && (vs.size () > 1))
                nits.pick (nit_too_many, es_error, ec_type, "one or two values expected; ignoring values from ", quote (vs.at (1)), " (7)");
            else if (vs.size () > 2)
                nits.pick (nit_too_many, es_error, ec_type, "one or two values expected; ignoring values from ", quote (vs.at (2)), " (8)");
            else if (res) return; }
        uq4 < T, SZ, F > :: status (s_invalid); }
    ::std::size_t size () const noexcept { return both_ ? 2 : 1; } };

template < e_type T, e_type U, class SZ, e_type P, int F = 0 > struct type_either_or_both_unordered : uq4 < T, SZ, F >
{   bool both_ = false;
    using uq4 < T, SZ, F > :: uq4;
    void set_value (nitpick& nits, const html_version& v, const ::std::string& s)
    {   vstr_t vs (uq4 < T, SZ, F > :: set_value (nits, v, s));
        if ((vs.size () > 0) && uq4 < T, SZ, F > :: good ())
        {   nitpick nuts, nets, knots;
            bool res = test_value < U > (nuts, v, vs.at (0), tidy_string < T > :: id ());
            if (! res)
            {   res = test_value < P > (nets, v, vs.at (0), tidy_string < T > :: id ());
                if (! res)
                {   nits.merge (nuts); nits.merge (nets); }
                else if (vs.size () == 1) return;
                else
                {   res = test_value < U > (knots, v, vs.at (1), tidy_string < T > :: id ());
                    if (! res) nits.merge (knots);
                    both_ = true; } }
            else
            {   nits.merge (nuts);
                if (vs.size () == 1) return;
                ...
                res = test_value < P > (nits, v, vs.at (1), tidy_string < T > :: id ()); }
            if ((! both_) && (vs.size () > 1))
                nits.pick (nit_too_many, es_error, ec_type, "one or two values expected; ignoring values from ", quote (vs.at (1)), " (10)");
            else if (vs.size () > 2)
                nits.pick (nit_too_many, es_error, ec_type, "one or two values expected; ignoring values from ", quote (vs.at (2)), " (11)");
            else if (res) return; }
        uq4 < T, SZ, F > :: status (s_invalid); }
    ::std::size_t size () const noexcept { return both_ ? 2 : 1; } };
...
template < e_type T, class SZ, e_type A, e_type B, e_type C, int F = 0 > struct type_abc_cba : uq4 < T, SZ, F >
{   using uq4 < T, SZ, F > :: uq4;
    void set_value (nitpick& nits, const html_version& v, const ::std::string& s)
    {   vstr_t vs (uq4 < T, SZ, F > :: set_value (nits, v, s));
        if (uq4 < T, SZ, F > :: empty ()) nits.pick (nit_empty, es_warning, ec_type, "bit minimalist, that (", type_name (T), ")");
        ...
        {   PRESUME (vs.size () > 0, __FILE__, __LINE__);
            bool good = true, a = false, b = false, c = false;
            for (auto x : vs)
            {   nitpick gnats, nets, knots;
                if ((! a) && test_value < A > (gnats, v, x, tidy_string < T > :: id ()))
                {   nits.merge (gnats); a = true; }
                else if ((! b) && test_value < B > (nets, v, x, tidy_string < T > :: id ()))
                {   nits.merge (nets); b = true; }
                else if ((! c) && test_value < C > (knots, v, x, tidy_string < T > :: id ()))
                {   nits.merge (knots); c = true; }
                else
                {   good = false; nits.merge (gnats); nits.merge (nets); nits.merge (knots); } }
            if (vs.size () > 3)
                nits.pick (nit_too_many, es_warning, ec_type, "ignoring values from ", quote (vs.at (3)), " (", type_name (T), ")");
            if (good) return; }
        uq4 < T, SZ, F > :: status (s_invalid); }
    ::std::size_t size () const noexcept { return 2; } };
...
template < e_type T, e_type P, class SEPARATOR, class SZ, int F = 0 > struct type_or_string_or_both : uq4 < T, SEPARATOR, F >
{   using uq4 < T, SEPARATOR, F > :: uq4;
    void set_value (nitpick& nits, const html_version& v, const ::std::string& ss)
    {   vstr_t vs (uq4 < T, SEPARATOR, F > :: set_value (nits, v, ss));
        if (uq4 < T, SEPARATOR, F > :: empty ()) nits.pick (nit_empty, es_warning, ec_type, "bit minimalist, that (", type_name (T), ")");
        else if (uq4 < T, SEPARATOR, F > :: good ())
        {   PRESUME (vs.size () > 0, __FILE__, __LINE__);
            bool bv = false, bs = false, good = true;
            for (auto s : vs)
            {   nitpick nuts, nets;
                if (compare_complain (nets, v, SZ :: sz (), s))
                {   nits.merge (nets);
                    if (bs)
                    {   good = false;
                        nits.pick (nit_too_many, es_error, ec_type, "too many of ", SZ :: sz ()); }
                    else bs = true; }
                else if (test_value < P > (nuts, v, vs.at (0)))
                {   nuts.merge (nuts);
                    if (bv)
                    {   good = false;
                        nits.pick (nit_too_many, es_error, ec_type, quote (s), ": too many values"); }
                    else bv = true; }
                else { nits.merge (nets); nits.merge (nuts); good = false; } }
            if (good && (bv || bs)) return; }
        uq4 < T, SEPARATOR, F > :: status (s_invalid); }
    ::std::size_t size () const noexcept { return 3; } };
...
{   using uq4 < T, sz_space_char, F > :: uq4;
    ...
    {   vstr_t vs (uq4 < T, sz_space_char, F > :: set_value (nits, v, s));
        ...
        {   PRESUME (vs.size () > 0, __FILE__, __LINE__);
            ...
            else if (vs.size () > 3) nits.pick (nit_too_many, es_warning, ec_type, "ignoring values from ", quote (vs.at (3)), " (", ::boost::lexical_cast < ::std::string > (vs.size ()), ",2)");
            if (good) return; }
        uq4 < T, sz_space_char, F > :: status (s_invalid); }
    ::std::size_t size () const noexcept { return 3; } };
    ...
    {   vstr_t vs (uq4 < T, SZ, F > :: set_value (nits, v, s));
        ...
        {   both_ = (vs.size () > 1);
            ...
            {   res = test_value < A > (nits, v, vs.at (0), uq4 < T, SZ, F > :: id ());
                ...
                if (vs.size () > 2) nits.pick (nit_too_many, es_warning, ec_type, "one or two values expected; ignoring additional values from ", quote (vs.at (2)), " (", ::boost::lexical_cast < ::std::string > (vs.size ()), ",6)"); }
            if (res) return; }
        uq4 < T, SZ, F > :: status (s_invalid); }
