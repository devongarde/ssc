...
void attributes :: verify_attributes (nitpick& nits, const html_version& v, element* pe, const ::std::string& lang)
{   VERIFY_NOT_NULL (pe, __FILE__, __LINE__);
    for (size_t i = 0; i < aar_.size (); ++i)
        ...
        {   const elem e (tag ());
            ...
            if (attr::spellcheck (static_cast < e_attribute > (i))) spell (nits, lang, aar_.at (i) -> get_string ()); } }
