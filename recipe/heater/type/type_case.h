...
{   if (v.xhtml () && (naam != s) && compare_no_case (naam, s))
        if (naam.find_first_of (UPPERCASE) != ::std::string::npos)
            nits.pick (nit_xhtml_enum_lc, ed_x1, "4.11. Attributes with pre-defined value sets", es_warning, ec_type, "enumerations must have matching case in ", v.report ());
        else nits.pick (nit_xhtml_enum_lc, ed_x1, "4.11. Attributes with pre-defined value sets", es_warning, ec_type, "enumerations must be lower cased in ", v.report ()); }
