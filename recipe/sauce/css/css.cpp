...
css::css (const html_version& v, const namespaces_ptr& namespaces, dst_ptr dst, page& p, const ::std::string& abs, bool state_version, bool snippet, int line)
    :   args_ (v, namespaces, p.css (), state_version, snippet, abs, dst), page_ (p), abs_ (abs), line_ (line)
{   if (abs.find (':') == ::std::string::npos) ndx_ = get_fileindex (abs); }

css::css (const html_version& v, const ::std::string& content, const namespaces_ptr& namespaces, dst_ptr dst, page& p, const ::std::string& abs, bool state_version, bool snippet, int line)
    :   args_ (v, namespaces, p.css (), state_version, snippet, abs, dst), page_ (p), abs_ (abs), line_ (line)
