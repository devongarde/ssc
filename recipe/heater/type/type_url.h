...
{   void set_value (nitpick& nits, const html_version& v, const ::std::string& s)
    {   if (v < html_5_0)
        ...
        if (type_master < t_url > :: good ())
            if (! value_.has_domain () || ! value_.has_absolute_path () || ! value_.has_protocol ())
            {   nits.pick (nit_schema_url, es_error, ec_type, quote (s), " must be an absolute URL identifying a standard microdata type (for example, those that start with 'http://schema.org/')");
                type_base < url, t_url > :: status (s_invalid); }
