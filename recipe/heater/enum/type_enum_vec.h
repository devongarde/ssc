...
{   original_ = s;
    nits.pick (nit_missing_set_value, es_error, ec_type, "Internal error: an enum vector is missing setvalue (", type_name (E), ")");
...
{   using enum_vec_base < ENUM, E, CATEGORY, INIT > :: enum_vec_base;
    ...
    {   bool bitten = false;
        ...
        ss << "\""; }
    void accumulate (stats_t* st) const
    {   enum_vec_base < ENUM, E, CATEGORY, INIT > :: accumulate (st); }
    void accumulate (stats_t* st, const e_element e) const
    {   enum_vec_base < ENUM, E, CATEGORY, INIT > :: accumulate (st, e); }
    void accumulate (stats_t* st, const element_bitset& e) const
    {   enum_vec_base < ENUM, E, CATEGORY, INIT > :: accumulate (st, e); }
