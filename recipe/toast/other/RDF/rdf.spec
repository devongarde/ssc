-f ../toast/other/RDF/rdf.conf

PASS ../toast/other/RDF/index.html
2 naughty_lang

PASS ../toast/other/RDF/rdf1-example.html
2 naughty_lang
11 unrecognised_value bad_namespace unrecognised_value unknown_rel link_rel_off

PASS ../toast/other/RDF/rdf2-example.html
4 naughty_lang
7 unrecognised_value unknown_rel link_rel_off
18 invalid_attribute_version wrong_version
22 unrecognised_value unknown_rel

PASS ../toast/other/RDF/rdf3-example.html
5 naughty_lang

PASS ../toast/other/RDF/rdf4-example.html
5 naughty_lang

PASS ../toast/other/RDF/rdf5-example.html
2 naughty_lang
