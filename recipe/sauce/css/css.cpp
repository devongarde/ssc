...
css::css (  const html_version& v, const namespaces_ptr& namespaces, dst_ptr dst, page& p, const ::std::string& abs, bool state_version, bool snippet, int line, const e_element styled,
            const element_bitset eb, const ::boost::filesystem::path dp)
    :   args_ (v, namespaces, p.css (), state_version, snippet, abs, dst, styled, eb), page_ (p), abs_ (abs), dp_ (dp), line_ (line)
{   if (abs.find (':') == ::std::string::npos) ndx_ = get_fileindex (abs); }

css::css (  const html_version& v, const ::std::string& content, const namespaces_ptr& namespaces, dst_ptr dst, page& p, const ::std::string& abs, bool state_version, bool snippet,
            int line, const e_element styled, const element_bitset eb, const ::boost::filesystem::path dp, const bool mdm)
    :   args_ (v, namespaces, p.css (), state_version, snippet, abs, dst, styled, eb), page_ (p), abs_ (abs), dp_ (dp), line_ (line)
{   if (args_.abs_.find (':') == ::std::string::npos) ndx_ = get_fileindex (args_.abs_);
    parse (content, true, mdm); }
...
void css::nick_nits (nitpick& nits)
{   for (auto t : args_.t_)
    {   nits.merge (t.nits_);
        t.nits_.reset (); } }
