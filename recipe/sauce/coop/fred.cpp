...
{   q_entry t (st_max);
    ...
    {   if (context.tell (es_detail))
            t.ticks_ -> pick (nit_detail, es_detail, ec_fred, "Fred ", ::std::this_thread::get_id (), " category ", t.stage_, " processing ", t.dir_ -> get_site_path ());
        d_q (t); }
    else if (t.ticks_ != nullptr)
        t.ticks_ -> pick (nit_kew_borked, es_error, ec_fred, "Fred ", ::std::this_thread::get_id (), " category ", t.stage_, ", got borked directory");
    return true; }
