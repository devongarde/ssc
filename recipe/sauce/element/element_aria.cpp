...
{   if (node_.version ().is_svg_2 ())
        if (a_.known (a_role))
            ...
            {   vint_t rv = a_.get_ints (a_role);
                for (auto r : rv)
                    if (is_default_role (node_.version (), node_.id (), static_cast < e_aria_role > (r)))
...
void element::examine_aria_brailleroledescription ()
{   PRESUME (a_.known (a_aria_brailleroledescription), __FILE__, __LINE__);
    if (a_.good (a_aria_brailleroledescription))
    {   if (has_attribute (tag (), a_aria_roledescription))
            if (a_.good (a_aria_roledescription))
                if (compare_no_case (a_.get_string (a_aria_roledescription), a_.get_string (a_aria_brailleroledescription)))
                    nits ().pick (  nit_aria_braille, ed_aria_1_3, "aria-brailleroledescription property",
                                    es_error, ec_aria, "ARIA-ROLEDESCRIPTION and ARIA-BRAILLEROLEDESCRIPTION should not have the same value");
        if (has_attribute (tag (), a_role))
            if (a_.good (a_role))
                if (compare_no_case (a_.get_string (a_role), a_.get_string (a_aria_brailleroledescription)))
                    nits ().pick (  nit_aria_braille, ed_aria_1_3, "aria-brailleroledescription property",
                                    es_error, ec_aria, "ARIA-ROLE and ARIA-BRAILLEROLEDESCRIPTION should not have the same value"); } }

void element::examine_aria_checked ()
{   PRESUME (a_.known (a_aria_checked), __FILE__, __LINE__);
    if (! has_attribute (tag (), a_checked))
        nits ().pick (  nit_aria_position, ed_aria_html, "4.2 Requirements for use of ARIA attributes in place of equivalent HTML attributes",
                        es_error, ec_aria, "(Only) use the checked attribute on any element that is allowed the checked attribute in HTML.");
    ...
    if (! has_attribute (tag (), a_colspan))
        ...
        if (a_.good (a_aria_colspan))
            if (a_.get_int (a_aria_colspan) == a_.get_int (a_colspan))
                nits ().pick (  nit_aria_position, ed_aria_html, "4.2 Requirements for use of ARIA attributes in place of equivalent HTML attributes",
                                es_warning, ec_aria, "ARIA-COLSPAN should not be used when COLSPAN is present");
            else nits ().pick ( nit_aria_position, ed_aria_html, "4.2 Requirements for use of ARIA attributes in place of equivalent HTML attributes",
    ...
    if (a_.known (a_hidden))
        if (a_.good (a_aria_hidden))
            if (a_.get_int (a_aria_hidden) == 1)
                nits ().pick (  nit_aria_position, ed_aria_html, "4.2 Requirements for use of ARIA attributes in place of equivalent HTML attributes",
                                es_error, ec_aria, "ARIA-HIDDEN must not be true when HIDDEN is until-found");
            else nits ().pick ( nit_aria_position, ed_aria_html, "4.2 Requirements for use of ARIA attributes in place of equivalent HTML attributes",
...
{   PRESUME (a_.known (a_aria_readonly), __FILE__, __LINE__);
    ...
    if (a_.known (a_contenteditable))
        if (a_.good (a_aria_readonly))
            if (a_.get_int (a_aria_readonly) == 1)
                if (a_.empty (a_contenteditable) || (a_.get_int (a_contenteditable) > 0))
                    nits ().pick (  nit_aria_position, ed_aria_html, "4.2 Requirements for use of ARIA attributes in place of equivalent HTML attributes",
...
void element::examine_aria_role ()
{   PRESUME (a_.known (a_role), __FILE__, __LINE__);
    vint_t rv = a_.get_ints (a_role);
    for (auto r : rv)
        if (r > role_any) ancestral_roles_ |= static_cast < e_aria_role > (r); }
    ...
    if (! has_attribute (tag (), a_rowspan))
        ...
        if (a_.good (a_aria_rowspan))
            if (a_.get_int (a_aria_rowspan) == a_.get_int (a_rowspan))
                nits ().pick (  nit_aria_position, ed_aria_html, "4.2 Requirements for use of ARIA attributes in place of equivalent HTML attributes",
                                es_warning, ec_aria, "ARIA-ROWSPAN should not be used where ROWSPAN is present");
            else nits ().pick ( nit_aria_position, ed_aria_html, "4.2 Requirements for use of ARIA attributes in place of equivalent HTML attributes",
