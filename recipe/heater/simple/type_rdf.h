...
template < > struct type_master < t_curies > : string_vector < t_curies, sz_space_char >
{   using string_vector < t_curies, sz_space_char > :: string_vector;
    static e_animation_type animation_type () noexcept { return at_url; }
    void set_value (nitpick& nits, const html_version& v, const ::std::string& s)
    {   string_vector < t_curies, sz_space_char > :: set_value (nits, v, s); } };
...
template < > struct type_master < t_prefix > : string_vector < t_prefix, sz_space_char >
{   using string_vector < t_prefix, sz_space_char > :: string_vector;
    void set_value (nitpick& nits, const html_version& v, const ::std::string& ss)
    {   ::std::string s (tart (ss));
        string_vector < t_prefix, sz_space_char > :: set_value (nits, v, s);
        if (! string_vector < t_prefix, sz_space_char > :: good ()) return;
        vabb_t vpre;
        nitpick nuts;
        string_vector < t_prefix, sz_space_char > :: status (parse_prefixes (nuts, v, s, box (), vpre, false));
        string_vector < t_prefix, sz_space_char > :: value_.clear ();
        for (auto p : vpre)
            string_vector < t_prefix, sz_space_char > :: value_.push_back (p.short_ + ": " + p.long_); } };
