...
template < class PRIKEY, class PUBKEY, const EVP_MD* (*CODE) (), class XYZ, void (*FN_CTX) (XYZ*) > class signature_stream 
{   
public:
    typedef ::std::shared_ptr < PRIKEY > pri_ptr;
    typedef ::std::shared_ptr < PUBKEY > pub_ptr;
private:
    void_ptr signature_;
...
template < class PRIKEY, class PUBKEY, const EVP_MD* (*CODE) (), class XYZ, void (*FN_CTX) (XYZ*) > class signature_file 
{   typedef signature_stream < PRIKEY, PUBKEY, CODE, XYZ, FN_CTX > sf_ss_t;
...
public:
    ...
    {   pri_ = typename sf_ss_t::pri_ptr (new PRIKEY (nits, pri, pw)); 
        pub_ = typename sf_ss_t::pub_ptr (new PUBKEY (nits, pub));
        if (pri_ && pub_)
