...
{   if (has_child () && (node_.version ().svg_version () > sv_none) && (node_.version ().svg_version () < sv_2_0) && ancestral_elements_.test (elem_svg))
    ...
    else if ((node_.version () >= html_jan05) && own_attributes_.test (a_style))
...
{   const bool alt_known = a_.known (a_alt);
    ...
    else if (! has_srcset) pick (nit_attribute_required, ed_jul25, "The IMG element", es_error, ec_attribute, "<IMG> requires SRC or SRCSET, or both");
