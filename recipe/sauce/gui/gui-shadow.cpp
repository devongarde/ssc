...
BEGIN_EVENT_TABLE (shadow_t, dialogue_t)
  EVT_BUTTON (wxID_HELP, shadow_t::OnHelpClick)
  EVT_CHECKBOX (check_site, shadow_t::OnShadow)
  EVT_BUTTON (button_add, shadow_t::OnIgnoreAdd)
  EVT_BUTTON (button_erase, shadow_t::OnIgnoreErase)
  EVT_BUTTON (button_rename, shadow_t::OnIgnoreRename)
  EVT_TEXT (text_ext, shadow_t::OnIgnoreTap)
  EVT_LISTBOX (list_ext, shadow_t::OnIgnoreExtension)
  EVT_LISTBOX_DCLICK (list_ext, shadow_t::OnIgnoreImpatience)
END_EVENT_TABLE ()

IMPLEMENT_CLASS (shadow_t, dialogue_t)

shadow_t :: shadow_t (wxWindow *mummy, wxWindowID id, const wxString& caption)
	: dialogue_t (wxPoint (SHADOW_X, SHADOW_Y), wxSize (SHADOW_WIDTH, SHADOW_HEIGHT))
{	Create (mummy, id, caption); } 

bool shadow_t :: Create (wxWindow *mummy, wxWindowID id, const wxString& caption)
{	if (! dialogue_t :: Create (mummy, id, caption, wxPoint (SHADOW_X, SHADOW_Y), wxSize (SHADOW_WIDTH, SHADOW_HEIGHT), SHADOW_STYLE)) return false;
	CreateControls ();
	return true; }
...
{	if (dialogue_t :: invalid ()) return;
	...
	{	check_site_ = GSL_OWNER (wxCheckBox) (new wxCheckBox (this, check_site, "Enable shadowing", wxDefaultPosition, wxDefaultSize, wxALIGN_RIGHT));
		if (check_site_ != nullptr)
		{	grid_check_ -> Add (check_site_, 0, wxALIGN_RIGHT|wxALL, 5);
	 		check_change_ = GSL_OWNER (wxCheckBox) (new wxCheckBox (this, wxID_ANY, "Only update changed files", wxDefaultPosition, wxDefaultSize));
			if (check_change_ != nullptr)
			{	grid_check_ -> Add (check_change_, 0, wxALL, 5);
	 			check_keep_ = GSL_OWNER (wxCheckBox) (new wxCheckBox (this, wxID_ANY, "Do not delete comments", wxDefaultPosition, wxDefaultSize, wxALIGN_RIGHT));
				if (check_keep_ != nullptr)
				{	grid_check_ -> Add (check_keep_, 0, wxALIGN_RIGHT|wxALL, 5);
	 				check_time_ = GSL_OWNER (wxCheckBox) (new wxCheckBox (this, wxID_ANY, "Insert generated time comment", wxDefaultPosition, wxDefaultSize));
					if (check_time_ != nullptr)
					{	grid_check_ -> Add (check_time_, 0, wxALL, 5);
	 					check_whitespace_ = GSL_OWNER (wxCheckBox) (new wxCheckBox (this, wxID_ANY, "Squish repeated whitespace", wxDefaultPosition, wxDefaultSize, wxALIGN_RIGHT));
						if (check_whitespace_ != nullptr)
						{	grid_check_ -> Add (check_whitespace_, 0, wxALIGN_RIGHT|wxALL, 5);
	 						check_ssi_ = GSL_OWNER (wxCheckBox) (new wxCheckBox (this, wxID_ANY, "Resolve Server Side Includes", wxDefaultPosition, wxDefaultSize));
							if (check_ssi_ != nullptr)
							{	grid_check_ -> Add (check_ssi_, 0, wxALL, 5);
								box_ -> Add (grid_check_, 3, wxEXPAND, 5);
								line_check_ = GSL_OWNER (wxStaticLine) (new wxStaticLine (this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxLI_HORIZONTAL));
	...
	{	stat_strat_ = GSL_OWNER (wxStaticText) (new wxStaticText (this, wxID_ANY, "Shadow Strategy:", wxDefaultPosition, wxDefaultSize, 0));
		...
		{	box_strat_ -> Add (stat_strat_, 0, wxALIGN_CENTER_VERTICAL, 5);
			...
			choice_strat_ = GSL_OWNER (wxChoice) (new wxChoice (this, wxID_ANY, wxDefaultPosition, wxDefaultSize, strat_count_, strategies, 0));
	...
	{	stat_site_ = GSL_OWNER (wxStaticText) (new wxStaticText (this, wxID_ANY, "Create shadow site in ", wxDefaultPosition, wxDefaultSize, 0));
		if (stat_site_ != nullptr)
		{	box_site_ -> Add (stat_site_, 0, wxALIGN_CENTER_VERTICAL, 5);
			dir_site_ = GSL_OWNER (wxDirPickerCtrl) (new wxDirPickerCtrl( this, wxID_ANY, wxEmptyString, "Select a " REPERTOIRE, wxDefaultPosition, wxDefaultSize, wxDIRP_DEFAULT_STYLE));
			if (dir_site_ !=  nullptr)
				box_site_ -> Add (dir_site_, 0, wxALL, 5); }
		box_ -> Add (box_site_, 1, wxALIGN_CENTER_HORIZONTAL, 5);
		line_site_ = GSL_OWNER (wxStaticLine) (new wxStaticLine (this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxLI_HORIZONTAL));
	...
	{	stat_comment_ = GSL_OWNER (wxStaticText) (new wxStaticText (this, wxID_ANY, "Insert this comment in pages:", wxDefaultPosition, wxDefaultSize, 0));
		if (stat_comment_ != nullptr)
		{	box_comment_ -> Add (stat_comment_, 0, wxALIGN_CENTER_VERTICAL, 5);
			text_comment_ = GSL_OWNER (wxTextCtrl) (new wxTextCtrl (this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, 0));
			if (text_comment_ != nullptr)
			{	box_comment_ -> Add (text_comment_, 0, wxALL, 5);
				box_ -> Add (box_comment_, 1, wxALIGN_CENTER_HORIZONTAL, 5);
				line_comment_ = GSL_OWNER (wxStaticLine) (new wxStaticLine (this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxLI_HORIZONTAL));
	...
	{	stat_cache_ = GSL_OWNER (wxStaticText) (new wxStaticText (this, wxID_ANY, "Shadow cache:", wxDefaultPosition, wxDefaultSize, 0));
		if (stat_cache_ != nullptr)
		{	box_cache_ -> Add (stat_cache_, 0, wxALIGN_CENTER_VERTICAL, 5);
			file_cache_ = GSL_OWNER (wxFilePickerCtrl) (new wxFilePickerCtrl( this, wxID_ANY, wxEmptyString, "Select a file", "*.ndx", wxDefaultPosition, wxDefaultSize, wxFLP_OVERWRITE_PROMPT | wxFLP_SAVE | wxFLP_USE_TEXTCTRL));
			if (file_cache_ != nullptr)
			{	box_cache_ -> Add (file_cache_, 0, wxALL, 5);
				box_ -> Add (box_cache_, 1, wxALIGN_CENTER_HORIZONTAL, 5);
				line_cache_ = GSL_OWNER (wxStaticLine) (new wxStaticLine (this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxLI_HORIZONTAL));
				if (line_cache_ != nullptr)
					box_ -> Add (line_cache_, 0, wxEXPAND | wxALL, 5); } } }

	ignore_.construct (*this, "ignore files with extensions:");
