...
{   typedef true_type has_int_type;
    ...
    {   numeric_value < t_base, unsigned int > :: set_value (nits, v, s);
        ...
        {   if (v.math_version () < math_2) return;
            ...
            if (v.math_version () >= math_4)
            {   nits.pick (nit_base, ed_math_4, "4.2.1.3 Non-Strict uses of <cn>", es_warning, ec_type, "rendering of a value of BASE greater than 36 is browser dependent");
                return; } }
...
template < > struct type_master < t_percentish > : type_master < t_fixedpoint >
{   using type_master < t_fixedpoint > :: type_master;
    static e_animation_type animation_type () noexcept { return at_percentage; }
    static bool is_numeric () { return true; }
    void set_value (nitpick& nits, const html_version& v, const ::std::string& s)
    {   ::std::string ss (trim_the_lot_off (s));
        ...
        {   type_master < t_fixedpoint > :: status (s_good);
            return; }
        nits.pick (nit_percent, es_error, ec_type, "expecting a positive value followed by '%'");
        type_base < double, t_fixedpoint > :: status (s_invalid); }
    ::std::string get_string () const
    {   return ::boost::lexical_cast < ::std::string > (value_) + "%"; }
    void shadow (::std::stringstream& ss, const html_version& , element* )
...
template < > struct type_master < t_r0_to_1 > : type_number_between < t_r0_to_1, 0, 1 >
{ using  type_number_between < t_r0_to_1, 0, 1 > :: type_number_between; };
