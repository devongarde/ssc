...
template < e_type T, flags_t HV, flags_t HE = NOFLAGS, flags_t H2 = NOFLAGS, flags_t H3 = NOFLAGS, flags_t H4 = NOFLAGS, flags_t H5 = NOFLAGS > struct required : public type_master < T >
{   using type_master < T > :: type_master;
    ...
    {   if (flagify < HV > :: accept (v.flags ()) &&    
            ...
            flagify < H5 > :: accept (v.ext5 ()))
                type_master < T > :: set_value (nits, v, s);
        else
        {   nits.pick (nit_version, es_error, ec_type, burble);
            type_master < T > status (s_invalid); } } };

template < e_type T, flags_t HV, flags_t HE = NOFLAGS, flags_t H2 = NOFLAGS, flags_t H3 = NOFLAGS, flags_t H4 = NOFLAGS, flags_t H5 = NOFLAGS > struct barred : public type_master < T >
{   using type_master < T > :: type_master;
    void set_value (nitpick& nits, const html_version& v, const ::std::string& s, const ::std::string& burble)
    {   if (flagify < HV > :: reject (v.flags ()) ||    
            ...
            flagify < H5 > :: reject (v.ext5 ()))
        {   nits.pick (nit_version, es_error, ec_type, burble);
            type_master < T > status (s_invalid); } 
        else type_master < T > :: set_value (nits, v, s); } };

template < e_type T, flags_t H2, flags_t H3, flags_t H4, flags_t H5 > struct css_req : public required < T, NOFLAGS, NOFLAGS, H2, H3, H4, H5 >
{   using required < T, NOFLAGS, NOFLAGS, H2, H3, H4, H5 > :: required; };

template < e_type T, flags_t H2, flags_t H3, flags_t H4, flags_t H5 > struct css_bar : public barred < T, NOFLAGS, NOFLAGS, H2, H3, H4, H5 >
{   using barred < T, NOFLAGS, NOFLAGS, H2, H3, H4, H5 > :: barred; };
